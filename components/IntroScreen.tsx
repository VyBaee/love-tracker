...
export default function IntroScreen({ showAuth, onStart, onBack, locale, setLocale, isDarkMode, setIsDarkMode }: IntroScreenProps) {
  ...
  return (
    <div className={`h-screen min-h-screen w-full flex flex-col transition-colors duration-500 relative overflow-hidden font-sans ${isDarkMode ? 'bg-slate-950' : 'bg-theme-50'}`}>
      ...
      {/* NAVBAR: Cố định vị trí trên cùng */}
      <nav className="w-full flex items-center justify-between p-6 lg:px-12 relative z-50 flex-shrink-0">
        <div className="flex items-center gap-2">
          {/* <div className="w-8 h-8 sm:w-10 sm:h-10 bg-theme-500 rounded-xl flex items-center justify-center shadow-lg shadow-theme-200/50">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {/* NÚT NGÔN NGỮ: LUÔN HIỆN TRÊN MOBILE */}
          ...
          {/* {!showAuth && (
            <button
              onClick={onStart}
              className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-full font-bold text-[11px] sm:text-sm shadow-sm border hover:shadow-md transition-all active:scale-95 ${isDarkMode ? 'bg-slate-800 text-slate-200 border-slate-700' : 'bg-white text-theme-600 border-theme-100'}`}
            >
              {t.loginBtn}
            </button>
          )} */}
      ...
      {/* ========================================================================= */}
      {/* VÙNG CHỨA CÁC MÀN HÌNH TRƯỢT DỌC (VERTICAL PUSH) */}
      {/* ========================================================================= */}
      <div className="flex-1 w-full max-w-7xl mx-auto relative overflow-hidden z-10">

        {/* CONTAINER CHÍNH: Chứa Intro + Auth xếp chồng lên nhau */}
        <div className={`transition-transform duration-1000 ease-in-out h-[200%] w-full flex flex-col ${showAuth ? '-translate-y-1/2' : 'translate-y-0'}`}>

          {/* 1. MÀN HÌNH INTRO (H-1/2, Full Width) */}
          <div className="h-1/2 w-full flex flex-col lg:flex-row items-center justify-center p-6 lg:px-12 relative">

            {/* Cột Text (Trái) */}
            <div className={`flex-1 flex flex-col items-center lg:items-start justify-center transition-all duration-700 ${showAuth ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            {/* Cột Mockup Ảnh (Phải - Ẩn trên Mobile) */}
            <div className={`flex-1 hidden lg:flex flex-col items-center justify-center relative transition-all duration-700 ${showAuth ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>

              {/* KHUNG DEMO: Dùng relative để neo lại cái kỷ niệm rơi */}
              <div className={`relative w-full max-w-md backdrop-blur-2xl border p-6 rounded-[2.5rem] shadow-2xl z-10 rotate-[-2deg] hover:rotate-0 transition-transform duration-500 ${isDarkMode ? 'bg-slate-900/40 border-slate-700/50 shadow-pink-900/10' : 'bg-white/60 border-white shadow-theme-900/10'}`}>
                ...
                {/* SỬA LỖI UI: Đặt thẻ kỷ niệm vào BÊN TRONG thẻ Demo có relative */}
          ...
          {/* 2. MÀN HÌNH ĐĂNG NHẬP (AUTH) (H-1/2, Full Width) */}
          {/* - Nằm ngay phía dưới màn Intro để chờ trượt lên - */}
          <div className="h-1/2 w-full flex items-center justify-center p-6 transition-all duration-700 flex-shrink-0">
